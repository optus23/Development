...
void j1App::FinishUpdate()
{
	...
	dt = seconds_since_startup - last_frame_ms;

	static char title[256];
	sprintf_s(title, 256, "Av.FPS: %.2f Last Frame Ms: %02u Last sec frames: %i Last dt: %f Time since startup: %.3f Frame Count: %lu ",
			  avg_fps, last_frame_ms, frames_on_last_update, dt, seconds_since_startup, frame_count);
	App->win->SetTitle(title);

	// TODO 2: Use SDL_Delay to make sure you get your capped framerate
	uint32 delay_ms;
	uint32 ms_frame_cap = 1000 / framerate_cap;

	if (last_frame_ms > ms_frame_cap)
		last_frame_ms = ms_frame_cap;
		
	delay_ms = ms_frame_cap - last_frame_ms;
	j1PerfTimer delay_timer;
	delay_timer.Start();

	SDL_Delay(delay_ms);
	LOG(" We waited for %i milliseconds and got back in %f", delay_ms, delay_timer.ReadMs());
