...
void j1Map::Draw()
{
	...
	for (p2List_item <MapLayer*> *map_layer = data.layers.start; map_layer; map_layer = map_layer->next)
	{
		for (int y = 0; y < data.height; ++y)
		{
			for (int x = 0; x < data.width; ++x)
			{
				int tile_id = map_layer->data->Get(x, y);
				if (tile_id > 0)
				{
...
TileSet* j1Map::GetTilesetFromTileId(int id) const
{
	...
	{
		if (id < tileset->next->data->firstgid)
		{
			break;
		}
		else
