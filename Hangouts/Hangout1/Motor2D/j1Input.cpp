...
bool j1Input::Awake(const pugi::xml_node &config_node)
{
	//_*_*_XML_*_*
	LOG("xml_node input interacts");
	App->config_node.child("input").child("NUM_KEYS").value();
	App->config_node.child("input").child("NUM_MOUSE_BUTTONS").value();
	App->config_node.child("input").child("LAST_KEYS_PRESSED_BUFFER").value();
